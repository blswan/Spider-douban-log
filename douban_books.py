...
class set_log(object):
	...
	def update_index_remain_type_and_book_url_list(self, index_type_url, index_log_name, remain_log_name, book_log_name):
	...
	def update_remain_type_and_book_url_list(self, remain_type_url, remain_log_name, book_log_name):
...
class parse_data(object):
	...
	def get_index_type_url(self, base_url):
		...
		types = base_html.xpath('//*[@id="content"]//div[@class="article"]//table[@class="tagCol"]//td/a/@href')
	...
	def get_remain_type_urls(self, index_type_url, index_log_name, remain_log_name, book_log_name, log):
		"""从所有标签首页链接获取书本链接"""
		...
		type_name =	re.findall('\/tag\/(.*)', index_type_url)[0]
		book_sectors = type_html.xpath('//*[@id="subject_list"]//li[@class="subject-item"]')
		max_page = ''
		for book_sector in book_sectors:
			comments_number_base = book_sector.xpath('.//div[@class="star clearfix"]/span[@class="pl"]/text()')[0]
			try:
				comments_number = re.findall('\d+', comments_number_base)[0]
			except:
				comments_number = 0
			try:
				score = book_sector.xpath('.//div[@class="star clearfix"]/span[@class="rating_nums"]/text()')[0]
			except:
				score = '暂无评分'
			type_book_data = {
				'tag': type_name,
		...
		db.close()
		try:
			max_page = type_html.xpath('//*[@id="subject_list"]//div[@class="paginator"]//a[last()]/text()')[0]
			...
			log.update_index_remain_type_and_book_url_list(index_type_url, index_log_name, remain_log_name, book_log_name)
		except:
			log.update_index_remain_type_and_book_url_list(index_type_url, index_log_name, remain_log_name, book_log_name)
		time.sleep(3 + 3*random.random())

	def get_book_urls(self, remain_type_url, remain_log_name, book_log_name, log):
		"""从所有标签首页以外链接获取书本链接"""
		print(f"{remain_type_url}开始爬取特定类型剩余信息")
		db = pymysql.connect(host=host, user=user, password=password, port=port, db=database)
		type_response = self.get_page(remain_type_url)
		type_html = etree.HTML(type_response)
		type_name =	re.findall('\/tag\/(.*)\?', remain_type_url)
		book_sectors = type_html.xpath('//*[@id="subject_list"]//li[@class="subject-item"]')
		book_urls = type_html.xpath('//*[@id="subject_list"]//li[@class="subject-item"]/div[@class="info"]/h2/a/@href')
		for book_sector in book_sectors:
			comments_number_base = book_sector.xpath('.//div[@class="star clearfix"]/span[@class="pl"]/text()')[0]
			try:
				comments_number = re.findall('\d+', comments_number_base)[0]
			...
			book_url = book_sector.xpath('./div[@class="info"]/h2/a/@href')[0]
			book_url_list.append(book_url)
		log.update_remain_type_and_book_url_list(remain_type_url, remain_log_name, book_log_name)
		db.close()
		time.sleep(3 + 3*random.random())
...
def run():
	...
	for index_type_url in index_type_url_list:
		try:
			info.get_remain_type_urls(index_type_url, 'index_type_url.log', 'remain_type_url.log', 'book_urls.log', log)
		except:
			None
	print(remain_type_url_list)
	for remain_type_url in remain_type_url_list:
		try:
			info.get_book_urls(remain_type_url, 'remain_type_url.log', 'book_urls.log', log)
		except:
			None
	print(book_url_list)
